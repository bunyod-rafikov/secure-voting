...
import com.brafik.samples.io.JavaxSmartCardInterface
 ...
 *
 */
class CardSimulator(runtime: SimulatorRuntime? = SimulatorRuntime()) :
    JavaxSmartCardInterface(runtime) {
    ...
    private class CardChannelImpl(override val card: CardImpl, private val channelNr: Int) :
        CardChannel() {
        ...
        override fun transmit(command: CommandAPDU?): ResponseAPDU? {
            return ResponseAPDU(card.transmitCommand(command!!.bytes))
        }

        override fun transmit(command: ByteBuffer?, response: ByteBuffer?): Int {
            val result = card.transmitCommand(CommandAPDU(command!!).bytes)
            response?.put(result)
            return result.size
        }
