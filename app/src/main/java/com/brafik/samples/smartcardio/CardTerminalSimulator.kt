...
import com.brafik.samples.smartcardio.CardTerminals
import com.brafik.samples.smartcardio.TerminalFactorySpi
...
//                params
    ...
    internal class CardTerminalsImpl(names: Array<String>) : CardTerminals() {
        ...
        override fun list(state: State?): List<CardTerminal> {
            ...
            synchronized(terminalStateMap) {
                val result: ArrayList<CardTerminal> =
                    ArrayList<CardTerminal>(simulatedTerminals.size)
        ...
        override fun waitForChange(timeout: Long): Boolean {
            return try {
                waitForLatch(terminalsChangeAutoResetEvent, timeout)
    ...
    internal class CardTerminalImpl(
        ...
        override fun waitForCardPresent(timeout: Long): Boolean {
            return try {
                waitForLatch(cardPresent, timeout)
            } catch (e: InterruptedException) {
                Thread.currentThread().interrupt()
                false
            }
        ...
        override fun waitForCardAbsent(timeout: Long): Boolean {
            return try {
                waitForLatch(cardAbsent, timeout)
            } catch (e: InterruptedException) {
                Thread.currentThread().interrupt()
                false
            }
